...
**Which model to test on — this matters:**

| Test Type | Purpose | Minimum Model |
|---|---|---|
| Primary validation | Confirms the skill works as designed | Tier 1 (Claude Sonnet 5, GPT-5.5, Gemini 3.1 Pro, etc.) |
| Graceful degradation | Confirms skill is still usable on lighter models | Tier 3 (Haiku, Gemma 3, smaller variants) |
| Production readiness | Confirms optimal output at frontier quality | Tier 1 Frontier (Fable 5, Opus 4.8, GPT-5.5) |

**A skill validated only on a Tier 3 model (e.g. claude-haiku-4.5) is showing
graceful degradation, not production readiness.** This is useful information
but should be labeled clearly in the Validation Record. The two CONDITIONAL
results in the v1.0 pilot skills were tested on Haiku — they may be model
capability gaps, not skill design gaps.

**Recommendation for all new skills:**
- Run primary validation on a Tier 1 model → record in main validation block
- Optionally run on Tier 3 → record in `additional_validation` block
- Label clearly which is which — the community needs both data points

**Record exactly which model you tested on** including version where known
(e.g. `claude-sonnet-5-20260401` not just `Claude Sonnet 5`).

**Cross-model testing (strongly encouraged):** If you test on multiple models,
add additional validation blocks to Component 13. This is one of the most
valuable contributions the project can receive — it tells the community
exactly where skills degrade and why, which informs the portability annotations.
